...
class VisualContextualPromptEncoder(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256, num_tokens: int = 4):
        ...
        # Cross-attention / MLP to project pooled support features into learned prompt tokens
    ...
    def forward(
        ...
        mask_down = F.interpolate(support_masks.float(), size=(H_f, W_f), mode="nearest")  # (K, 1, H_f, W_f)

        # Weight mask by NDVI if provided to emphasize healthy vegetation pixels
        if support_ndvi is not None:
            ...
            mask_down = mask_down * weight

        # Masked Average Pooling across K support shots
        mask_sum = mask_down.sum(dim=(2, 3), keepdim=True).clamp(min=1e-6)
        masked_feats = (support_embeddings * mask_down).sum(dim=(2, 3), keepdim=True) / mask_sum  # (K, C, 1, 1)

        # Average across the K support examples to form the class prototype
        prototype = masked_feats.mean(dim=0).view(1, C)  # (1, C)

        # Project prototype into contextual prompt tokens
        tokens = self.proj(prototype).view(1, self.num_tokens, self.embed_dim)  # (1, num_tokens, embed_dim)
